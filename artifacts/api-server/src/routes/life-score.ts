...
export async function computeLifeScore(userId: number) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const yScores = await db.select().from(lifeScoresTable)
      .where(and(eq(lifeScoresTable.userId, userId), eq(lifeScoresTable.scoreDate, yDate))).limit(1);
    const delta = yScores.length ? score - yScores[0].score : 0;

    const todayStr = new Date().toISOString().split("T")[0];
    const existing = await db.select().from(lifeScoresTable)
      .where(and(eq(lifeScoresTable.userId, userId), eq(lifeScoresTable.scoreDate, todayStr))).limit(1);
...
router.get("/history", async (req, res) => {
  try {
    ...
    const result = history.map(h => ({ date: h.scoreDate, score: h.score }));
